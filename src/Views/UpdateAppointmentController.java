...
public class UpdateAppointmentController implements Initializable {
    @FXML private Label upAppTitleLabel;
    ...
    @FXML void saveButtonPushed(ActionEvent event) throws IOException {
        ...
        if (ESTStartTimeValue < 0 || localStartDateTime.getDayOfWeek() == DayOfWeek.SATURDAY || localStartDateTime.getDayOfWeek() == DayOfWeek.SUNDAY) {
           ...
           DBQuery.updateAppointment(Integer.parseInt(appIDTF.getText()), appTitleTF.getText(), appDescriptionTF.getText(),appLocationTF.getText(),
                   appTypeTF.getText(), UTCStartDateString, UTCEndDateString, LoginScreenController.currentUser, selectedCustomer.getCustomerID(), selectedUser.getUserId(),selectedContact.getContactId());
    ...
    @FXML void cancelButtonPushed(ActionEvent event) throws IOException {
        ...
        alert.setHeaderText(rb.getString("updateAptErrorTitle"));
        alert.setContentText(rb.getString("updateAptError"));
        alert.showAndWait();
    ...
    public boolean checkForOverlaps(){
        ...
        for (int i = 0; i < allAppointments.size(); i++) {
            if (allAppointments.get(i).getAppId() != Integer.parseInt(appIDTF.getText())) {
    ...
    public  void getAppointmentData(Appointment appointment) {
        ...
        String start = appointment.getAppStart();
        String end = appointment.getAppEnd();
        ...
        appIDTF.setText("" + appointment.getAppId());
        appTitleTF.setText(appointment.getAppTitle());
        appDescriptionTF.setText(appointment.getAppDescription());
        appLocationTF.setText(appointment.getAppLocation());
        appTypeTF.setText(appointment.getAppType());
        ...
        for (int i = 0; i < allContacts.size(); i++) {
            if (allContacts.get(i).getContactId() == appointment.getAppContactId())
                contactCB.setValue(allContacts.get(i));
        }
        for (int i = 0; i < allCustomers.size(); i++) {
            if (allCustomers.get(i).getCustomerID() == appointment.getAppCustId())
                customerCB.setValue(allCustomers.get(i));
        }
        for (int i = 0; i < allUsers.size(); i++) {
            if (allUsers.get(i).getUserId() == appointment.getAppUserId())
                userCB.setValue(allUsers.get(i));
        }
    }
    ...
    public void setLanguage(){
        ...
        if (Locale.getDefault().getLanguage().equals("fr") || Locale.getDefault().getLanguage().equals("en")) {
            upAppTitleLabel.setText(rb.getString(upAppTitleLabel.getId()) + ":");
